...
class Model_app extends CI_Model
{
    ...
    // {
    //     $a = $this->
    //     $query = $this->db->query("select email from karyawan where"++"");
    ...
    {
        $query = $this->db->query('SELECT A.nama, A.nik, A.email, A.alamat, A.jk, C.nama_bagian_dept, B.nama_jabatan, D.nama_dept
    ...
    public function datalist_klasifikasi()
    {

        $query = $this->db->query("SELECT D.nama, D.jk, F.nama_dept, A.status, A.id_ticket, A.tanggal, A.tanggal_solved, B.nama_sub_kategori, C.nama_kategori, G.feedback
                                   FROM ticket A 
                                   LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                                   LEFT JOIN kategori C ON C.id_kategori = B.id_kategori
                                   LEFT JOIN karyawan D ON D.nik = A.reported
    ...
    public function detailinformasi($id = NULL)
    {

        $query = $this->db->query("SELECT A.tanggal, A.subject, A.pesan, C.nama, A.id_informasi
                                   FROM informasi A 
                                   LEFT JOIN karyawan C ON C.nik =  A.id_user
                                   WHERE (A.status = 1 && A.id_informasi = $id)");
        return $query->row();
    }
    ...
    {
        $query = $this->db->query('SELECT A.username, A.email, A.level, A.id_user, B.nik, B.nama, A.password, C.id_dept, D.nama_dept 
    ...
    public function dropdown_bag_departemen()
    {
        $sql = "SELECT * FROM Bagian_departemen ORDER BY nama_bagian_dept";
        $query = $this->db->query($sql);
    ...
    public function getemailapproval($ticket)
    {
