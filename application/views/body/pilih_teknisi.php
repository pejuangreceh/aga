			...
			<?php
			function ubah_ke_teknisi($string)
			{
				// Mengambil angka terakhir dari string
				$last_number = (int) substr($string, -1);

				// Mengubah 'T0001' menjadi 'Teknisi 1'
				$result = "Teknisi " . $last_number;
									...
									<form method="post" action="<?php echo base_url(); ?><?php echo $url; ?>">
										...
										<div class="form-group">
											...
											<select name="id_teknisi" id="id_teknisi" class="form-control">
												<?php foreach ($presentase_rekomendasi_teknisi as $key => $value) { ?>
													<option value="<?= $key; ?>"><?= ubah_ke_teknisi($key); ?></option>
												<?php } ?>
											</select>
