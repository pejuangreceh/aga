...
class Home extends CI_Controller
{
	...
	function index()
	{
		...
		// jumlah kerusakan 
		$sql_jumlah_kerusakan = "SELECT ticket.id_sub_kategori,sub_kategori.nama_sub_kategori , COUNT(ticket.id_sub_kategori) AS jumlah FROM ticket LEFT JOIN sub_kategori ON sub_kategori.id_sub_kategori = ticket.id_sub_kategori";
		$row_total_kerusakan = $this->db->query($sql_jumlah_kerusakan)->row();

		$sql_jumlah_kerusakan = "SELECT ticket.id_sub_kategori,sub_kategori.nama_sub_kategori , COUNT(ticket.id_sub_kategori) AS jumlah FROM ticket LEFT JOIN sub_kategori ON sub_kategori.id_sub_kategori = ticket.id_sub_kategori GROUP BY ticket.id_sub_kategori ORDER BY jumlah DESC";
		$row_kerusakan = $this->db->query($sql_jumlah_kerusakan)->row();

		//jumlah kantor paling banyak rusak
		$sql_jumlah_kantor = "SELECT t.email_user,u.kantor, COUNT(u.kantor) AS jumlah FROM ticket t LEFT JOIN user u ON t.email_user = u.email";
		$row_total_kantor = $this->db->query($sql_jumlah_kantor)->row();

		$sql_jumlah_kantor = "SELECT t.email_user,u.kantor, COUNT(u.kantor) AS jumlah FROM ticket t LEFT JOIN user u ON t.email_user = u.email GROUP BY u.kantor ORDER BY jumlah DESC";
		$row_kantor = $this->db->query($sql_jumlah_kantor)->row();
		...
		$data['jml_rusak'] = $row_kerusakan->jumlah;
		$data['nama_rusak'] = $row_kerusakan->nama_sub_kategori;
		$data['jml_kantor'] = $row_kantor->jumlah;
		$data['nama_kantor'] = $row_kantor->kantor;
